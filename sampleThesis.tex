...
Genomic instability is a hallmark of cancer \cite{Hanahan2011-ni} and numerous studies have proved the ubiquity of genomic variants in all cancer tissues \cite{pcawg_marker2020-yi} (cite TCGA too). The reportoire of somatic variants in cancer genomes is the result of mutagenesis, DNA repair, and evolution (cite some Mike Stratton review). The goal of studying them is to find generic patterns out of large panels of tumors (each can be regarded as an independent outcome), infer possible mechanisms from which specific classes of variants arise, and ultimately understand cancer etiology. 

Even though the idea that abnormal chromosomes as an intrinsic property of cancer has been proposed \cite{Boveri2008-rl} and verified for more than a century, the advent of massively parallel sequencing in the past decade has been revolutionary in expanding the reportoire of somatic variants in unprecedented details (cite Matthew Meyerson review). Enrichment for specific DNA sequences for sequencing, including gene panels and whole exome sequencing (WES), are done routinely in many clinical settings (cite exome paper from GDAN bibliography). While currently less widely used in clinic due to higher cost and analytically more challenging (cite Olivier review recently), whole genome sequencing (WGS) provides a hollistic profile of all single nucleotide variants (SNV), small insertions and deletions (INDEL), and structural variants (SV), thanks to the unbiased sampling of DNA fragments over the whole genome. No doubt that more WGS data and more sophiticated analyses will largely improve our understanding of the origin of cancer.

Such studies answer fundamental biological questions regarding the relationships between indogeneous, exogeneous mutagens, and DNA repair mechanisms in soma (cite Marcin, Lerner, Nuria, Lawrence, mutation signature with mutagen experiments and analysis, Florian? Nik-Zainal). With better understanding of mutation propensity of different parts of the genome in different tissues, environments, and genetic backgrounds, we can also better approximate the null distribution of mutation density and more precisely pinpoint the loci undergoing positive selection, termed \textit{drivers} (cite FishHook, MutSigCV, dndscv). On top of that, they also bring profound impact onto the prevention (cite UV, smoke damage, BRCA1 familial risk assessment) and treatment (cite TMB, MSI biomarker for immunotherapy; HRDetect for PARPi) of cancer.



One of the most prominent examples is the distillation of \textit{mutation signatures} from collections of mutation profiles of various cancer genomes (or exomes).

% large numbers of cancer genomes revealing ubiquitous yet heterogeneous patterns ofd
...
Despite inferred orthogonally, junctions and CNAs are two facets of the same latent genome structure. 
...
\section{A practical toolset for the analysis of genome graphs is needed}
...
\section{Visualizing Structural variants for clinical oncology}
Another major obstacle between genome graphs and its wider adoption by the research community is the lack of proper visulization tools. Projection of data onto reference genome coordinates, or genome browsers, have been the quintessential tool of genomics \cite{}, yet for Structural variants (SV), most of them are constrained to plot one or two regions at a time. Cancer genomes, however, can contain complex Structural variant wherein a wide range of loci can be inter-stitched and the interpretation of an event is often dependent on its many individual rearrangement junctions (RJ).
