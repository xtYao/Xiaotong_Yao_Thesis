...
% Admin feedback
% 1. Date on title page must be month of conferral. Confirm with program coordinator and/or Registrar. Month must be written in full, IE "December". 
% 2. Please adjust widowed lines at bottom of page 6, etc, and orphaned line at top of page 10, etc. 
% 3. When noting citation for a published chapter, include the asterisk after chapter title and citation text at bottom of first chapter page. See formatting guidelines for more details. 
% 4. While not required, it is advised to either include the figure number and title in the caption paragraph or single-space the caption paragraph so as to avoid confusion. Please be sure all captions have consistent justification, either center or left. 
% 5. Text in figure is too small; all lettering must be readable even when reduced 25%, including axes and legends. Please enlarge the text/figure to make the text readable. Alternatively, you can separate and enlarge portions of the figures, rotate them 90 degrees, or adjust the background color for better contrast. Please see guidelines if the figure takes up or exceeds a single page. (Figures 2.3, 2.11, 3.20, 4.1, 4.2, 4.12, 5.3, 5.4, 5.22 
% 6. When a figure/table is rotated, any text appearing on that page (EG. the caption) must be rotated in the same orientation. (Tables 2.1, 3.2, 4.2, Figure 4.11, 4.13 
% 7. When a figure/table takes up or exceeds one page, the caption must be placed alone on the preceding page. Any subsequent pages of the figure must begin with begin with "Figure/Table X (Continued)". (Table 3.1 
% 8. Fix margins for Figure 3.17, some of the figure is off the page. 
% 9. Caption overlaps with page number (caption to Figures 3.21 on page 93 and 4.14 on page 124). Please revise. 
% 10. Caption for Figure 4.3 extends over the bottom margin, as does Figure on page 167. Please revise. 
% 11. Text on page 194 extends over right margin, please revise. 
...
\begin{figure}
    ...
    \caption{\textbf{Genome graph-derived features define biologically distinct patient groups}}{Heatmap of normalized junction burden across 2,487 unique patients yields 14 clusters named after their most prevalent event types (CT, TYF [tyfonas], BFB [BFBC], BR [BFBC, rigma], SPRS [sparse], CP [chromoplexy], TIC, TRA [translocations], DM [double minute], PYR [pyrgo], DUP [various duplications], DDT [deletion, duplication, and TIC], and INVD [inverted duplications]).}
...
\begin{figure}
    ...
    \caption{\textbf{Control group with exogeneous \textit{TERT} show no CNA.}}
...
\begin{figure}[h!]
    \centering
    \includegraphics[angle=90,width=0.75\linewidth]{figures/600ppi/tc_heatmap_low.png}
...
\begin{figure}
    ...
    \caption{\textbf{Phylogeny reconstructed based on SNVs.}}{From left to right: phylogenetic tree; heatmap of variant allele fractions (VAF) for SNVs detected among two or more clones, private SNVs are omitted for simplicity; copy number segmentation; junction presence.}
    \label{fig:tc_phylo}
\end{figure}
...
\begin{figure}
    ...
    \caption{\textbf{Clustered GC mutations around junction breakends.}}{g SNV patterns as a function of GC vs AT reference nucleotide context, with y-axis showing the logarithm of the inter SNV distance. Highlighted region represents a GC mutation cluster that is found across all four clones in the BFB cluster which harbour junction j2.}
...
\begin{figure}
    ...
    \caption{\textbf{Over-expression of genes amplified in the DM.}}{2 of 5 (\textit{UBL3} and \textit{LIG4}) showed marked over-expression relative to RNA-seq data for the full TCGA LUAD cohort.}
