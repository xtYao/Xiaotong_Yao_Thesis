...
Genomic instability is a hallmark of cancer \cite{Hanahan2011-ni} and genomic DNA sequencing of tumour tissues have proved the ubiquity of genomic variants \cite{pcawg_marker2020-yi} (cite TCGA too). Much of early cancer genomics is spent in capturing specific alterations as the cause of malignant transformation, termed \textit{drivers} (cite \cite{Cancer_Genome_Atlas_Research_Network2008-fp}), on the premise that excessively recurrently occuring events from independent tumors must be positively selected during the somatic evolution that led to the tumor cell clones (\cite{Lawrence2013-zx}). 

Even though the idea that abnormal chromosomes as an intrinsic property of cancer has been proposed \cite{Boveri2008-rl} and verified for more than a century, the advent of massively parallel sequencing in the past decade has been revolutionary in expanding the reportoire of somatic variants in unprecedented details including single nucleotide variants (SNV), small insertions and deletions (INDEL), and structural variants (SV)(cite Matthew Meyerson review).

Soon, the data became clear that somatic mutations are not randomly distributed over the genome or tumor types, and knowing why that is will 

In fact, the whole reportoire of somatic variants in cancer genomes is the result of mutagenesis, DNA repair, and evolution \cite{Stratton2009-ty}. To find generic patterns out of these variant profile of tumors can help infer possible mechanisms from which specific types of variants arise, and ultimately understand cancer etiology.
...
% The points of studying variant patterns at all
Studies of genomic variant profiles combine various features of variants, exemplified by substitution type, spatial distribution (clustered mutations APOBEC), multiscale sequence context (tri-nucleotide signatures; mesoscale palindromes), tissue specificity (), to answer fundamental biological questions regarding the relationships between indogeneous, exogeneous mutagens, and DNA repair mechanisms in soma (cite Marcin, Lerner, Nuria, Lawrence, mutation signature with mutagen experiments and analysis, Florian? Nik-Zainal). (Describe these examples).
...
On top of that, they also bring profound and direct impact onto the prevention (cite UV, smoke damage, BRCA1 familial risk assessment), diagnosis (cite inferring cancer cell of origin from mutation signatures, Gad?),  and treatment (cite TMB, MSI biomarker for immunotherapy; HRDetect for PARPi) of cancer.
