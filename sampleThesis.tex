...
% Final edits:
% Write a good acknowledgement
% Remove Sally text and figures and just refer to them
% include the clinical report system in the Conclusions
% rinse once to fix all citations

% 1. Confirm conferral date with Registrar; update title page if necessary.
% 2. Please change title of "Contents" to "Table of Contents"
...
% 4. When chapters are based on published materials, it must be noted appropriately. Please see guidelines for more information.
...
% 5. Captions for tables must appear above the table. (Table B.1)
...
\begin{acknowledgements}
    This thesis is founded by my mentor Dr. Marcin Imielinski. In 2016, I was pondering whether a particular recurrent SV events in prostate cancer genomes is just a variant calling artifact, when Marcin generously broke me free from the existing framework of counting junctions like mutations, by revealing a possibility where all SVs in a genome can be unified in one graph. The elegance of this formulation immediately captured my interest, but it was not until after numerous afternoons sitting down with Marcin, browsing through thousands of genome graphs, improving our models over and over, that I finally felt I grasped the significance of this work. 
...
The catalogue of somatic variants in a cancer genome is the result of mutagenesis, DNA repair, and evolution \cite{Stratton2009-ty}. The goal of studying genomic variant patterns is to find generic classes, infer mechanisms from which specific classes of variants arise, and ultimately understand cancer etiology.
...
First of all, these studies are driving the insights into the basic biology of genome maintanence and DNA repair. One of the most critical and extensively studies DNA repair pathway, homologous recombination (HR) Later, by adapting an \textit{Escherichia coli} replication fork barrier (RFB) in human cells, Willis et al. proposed that BRCA1 not BRCA2 is responsible for suppressing the replication restart-bypass mechanism tha result in microhomology-mediated tandem duplications \cite{Willis2017-bw}. 
...
To represent coherent double stranded genomic DNA, or karyotype, we need to annotate genome graphs (both vertices and edges) with non-negative integer copy numbers, and because of the linear structure of DNA, we find that these copy numbers need to follow \nameref{eq:junction_balance_constraint}. Such graphs are named junction-balanced genome graphs. In \nameref{chap:gg}, we will show that junction balance arises naturally from compressing all the linear alleles.
...
The most important goal of sequencing cancer genomes is to pinpoint the somatic alterations responsible for the malignant phenotype, or \textit{drivers}. To achieve that, the main signal to rely on is the recurrent alteration of a particular region of the genome from independent patients tumors. Since each patient's tumor can be regarded an independent clonal expansion, if a particular genomic region is altered in a cohort more than expected from a null distribution where the region is not positively selected, it can be nominated as selected in these cancers. 

Having discussed how our genome graph paradigm can represent complex genome structures and be used to study SV patterns, I will take it a step further and show how it can improve the whole genome analysis of tumors by better modeling of the rearrangements.



The Cancer Genome Atlas (TCGA) is one of the earliest large-scale cancer sequencing projects that has laid the foundation for molecular taxonomy of cancer.
...
with identified recurrent somatic variants, transcriptomic or proteomic subtypes, aberrant signaling pathways. Yet, limited by the cost and difficulty to analyze, WGS was not the primary choice of genome profiling for the most part. Whole exome sequencing (WES), which is built to capture all of the known coding sequences, has been an essential tools for researcher and oncologists alike. However, by definition, WES will miss regions outside its scope. After we approach the full capacity of WES on defining driver events in cancer cohorts, there are usually a subset tumors in which we cannot define a clear driver.


In the last Chapter \ref{chap:gdan} and Concluding Remarks \ref{chap:conclude}, we shall discuss about how our genome graph paradigm achieves a more comprehensive characterization of whole genomes of clinically relevant cancer samples.



I will demonstrate how to integrate it into a comprehensive whole genome characterization of a specific cancer cohort. 
...
\begin{figure}
    ...
    \caption{\textbf{Estimted versus gold standard CN in all successful runs.}}{Pearson correlation coefficients $\rho$ using all points are labeled.}
...
\begin{figure}
    ...
    \caption{\textbf{Reconstructed genome graphs around HER2 in HCC1954.}}{Example of a putative BFBC event in HCC1954 and the reconstruction by five different methods (top five) and the binned read coverage transformed in CN space (bottom).}
...
\textit{\textbf{Individual contributions:}} The Cancer Genome Atlas consortium conceptualized, initiated the project, and collected data. Jian Carrot-Zhang, 
...
\begin{figure}
    ...
    \caption{\textbf{An example of EGFR amplification and over-expression by BFB.}}{Top panel, EGFR RSEM among RNA-seq of LUAD samples. Bottom panel, purity-adjusted copy number and SV junctions (red lines) support a BFB cycles event underlying the amplification. Lower panels indicate WGS read depth and gene location in the region. CN, copy number.}
...
% (chromothripsis \cite{stephens2011}, chromoplexy \cite{baca2013}, templated insertion chains (TIC) \cite{spies2017,Li2020sv}, breakage fusion bridge cycles (BFB cycless) \cite{Zakov:2013cm}), 
