...
As we have discussed in the first chapter (\ref{sec:philo}), the discovery of mutation signatures can be seen as essentially extracting latent themes from a collection of samples each represented as counts of variants of distinct classes. There are exciting new developments in the algorithms that given such a count matrix can extract signatures, especially ones that balance the more abundant and rarer classes \cite{Funnell2019-ti} and ones that are flexible enough to integrate genomic variables \cite{Vohringer2021-bv}. However, the more fundamental questions lie in the construction of this count matrix, namely: 1) to find new patterns in new samples, and 2) expand and refine the vocabulary of variant classes.

While the completion of PCAWG marked an important milestone of whole genome profiling of primary cancers \cite{pcawg_marker2020-yi}, that for the first time, all variants including SVs from various cancer types are harmonized and available for a systematic analysis, it may seem granted the number of WGS-profiled tumor samples keep growing in the near future. In fact, there has been a study of substitution mutation signatures in a cohort of more than 12000 prospectively collected new pan-cancer WGS samples \cite{Degasperi2022-am}, the largest to date. Yet, there is plenty to consider on what type of samples to sequence next to optimize the gain of insights and clinical benefit. 
...
, as we have pointed out throughout this thesis, the more conceptually limiting step for SV signature discovery is the lack of adequate variant taxonomy that can precisely represent individual complex events of many junctions. 
