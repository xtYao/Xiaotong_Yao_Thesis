...
As we have discussed in the first chapter (\ref{sec:philo}), the discovery of mutation signatures can be seen as essentially factorizing an independent samples by variant classes count matrix, which allows two fundamental paths to advance: profile new samples and improve the taxonomy of variants. With the fast pace at which new cancer WGS studies are carried out, there seems little doubt the next few years the accumulated number of WGS-profiled tumor samples will step into tens of thousands. In fact, there has been a study of substitution mutation signatures in a cohort of more than 12,000 prospectively collected new pan-cancer WGS samples \cite{Degasperi2022-am}, the largest to date. However, 

On the other hand, as we have pointed out throughout this thesis, the more conceptually limiting step for SV signature discovery is the lack of adequate variant taxonomy that can precisely represent individual complex events of many junctions. 
