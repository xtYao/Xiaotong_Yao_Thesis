...
Such studies combine various features of variants, including substitution type, spatial distribution (strand-coordinated clustered mutations APOBEC, CITE Gordenin?), multiscale sequence context (tri-nucleotide signatures; mesoscale palindromes), replication timing, transcription level (CITE Lawrence, Polak), 3-dimensional chromatin structures (CITE Akdemir Nat Gen 2020; Garcia-Nieto 2017), tissue specificity (lineage-specific INDELs from Marcin, negative correlation between frequency and origin from Paz), to answer fundamental biological questions regarding the relationships between indogeneous, exogeneous mutagens, and DNA repair mechanisms in soma (cite Marcin, Lerner, Nuria, Lawrence, mutation signature with mutagen experiments and analysis, Florian? Nik-Zainal). They are creating wide-reacing impacts on at least three fronts.

% THE SIGNIFICANCE TO STUDY VARIANT PATTERNS!!!!!!!!!!!!!!!
% 1, biology of genome maintanence
First of all, these studies provide insights into the basic biology of genome maintanence. One of the most critical and extensively studies DNA repair pathway, homologous recombination (HR) Later, by adapting an \textit{Escherichia coli} replication fork barrier (RFB) in human cells, Willis et al. proposed that BRCA1 not BRCA2 is responsible for suppressing the  (CITE Scully paper) \cite{Willis2017-bw}.
...
Second, more textured understanding of the heterogeneous background somatic mutation rate of the genome in various tissue origins and environments improves the detection of drivers. As one of, if not the, most important goals of cancer genomics, determining the exact set of alterations (drivers, out of a vast sea of somatic variants) that cause malignant transformation of cells has been heavily guided by the analysis of significantly recurrently mutated sites/regions/genes. Very early on, we have realized that without a well-calibrated background mutation distribution, enormous amount of false positives will surely render cancer genome sequencing efforts futile \cite{Getz2007-wf}. Many statistical models are used to correct for background mutation frequencies. MutSigCV xxx. FishHook xxx. dNdScv xxx.

% 3, clinical benefit
Last but not least, they are bringing profound clinical impact to the prevention , diagnosis, and treatment of cancer. 

Using the positive association between smoking behavior of the donors and the observed SNV signature 4 burden, Alexandrov et al. estimated each cigarette smoked roughly equivalent with x somatic mutations accumulated in lung tissue. This confirms and complements decades of public health and epidemiology conclusions about the causal link between smoking and elevated cancer risks. 

(cite UV, smoke damage, BRCA1 familial risk assessment)

mutation signatures combined with sophisticated machine learning techniques are shown to predict the tissue of origin with  (cite inferring cancer cell of origin from mutation signatures, Gad?).

treatment (cite TMB, MSI biomarker for immunotherapy; HRDetect for PARPi) of cancer.
...
Merely just over a decade young, genomic profiling of cancer has already revolutionized our approach to study cancer and will surely continue to grow, fueled by the ever decreasing sequencing cost and the pressing need to optimize clinical benefits \cite{Stratton2009-ty}. 

The future of material, 1) non-cancerous soma mutation landscapes (either healthy tissue or other diseases), 2) multiple spatio-temporal sampling, 3) single cell DNA sequencing. "All of these can benefit from genome graph paradigm".

The future of technology, 1) long-read sequencing, 2) optical mapping.
...
The future of SV pattern and mutagenesis: 1) data driven feature engineering on phased graphs, 2) better bottom-up theoretical improvements and forward simulation, 3) experimental models of complex SVs, 4) 

The future of precision oncology, 1) systematic discovery of enhancer hijacking, 2) somatic "eQTL", 3) SV patterns as better biomarkers, 4) recurrence models of proximities.


1) I defined genome graph to represent the complex Structural variants in cancer genomes. I showed junction-balanced genome graphs (JBGG) arise naturally from the fact that DNA is linear, and that junction CNs and segmental CNs can be simultaneously inferred with superior fidelity from WGS using our MIQP-based algorithm Junction Balance Analysis.
